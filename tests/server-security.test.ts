...
describe("server host security", () => {
  ...
  test("rejects non-loopback bindings without gateway api keys", () => {
    ...
    expect(resolveServerBinding("0.0.0.0", false, true).networkExposed).toBe(
      true,
    )
  })
...
describe("network-exposed server policy", () => {
  test("requires authentication for protected routes when no keys exist", async () => {
    ...
    app.get("/protected-probe", (c) => c.text("ok"))
    const response = await app.request(
      "http://gateway.example.com/protected-probe",
    )

    expect(response.status).toBe(401)
  })

  test("allows an explicitly unauthenticated non-loopback listener", async () => {
    const app = createServer({
      networkExposed: true,
  ...
  test("allows loopback CORS but not foreign origins by default", async () => {
    const app = createServer()
    const foreign = await app.request("http://127.0.0.1/", {
      headers: { origin: "https://client.example.com" },
    })
    const loopback = await app.request("http://127.0.0.1/", {
