...
gateway key, uses the saved GitHub login instead of ambient token variables, and
...
Because that catalog also contains GPT models for Codex, set Claude Desktop's **Models** list (`inferenceModels`) to the Claude IDs your seat offers. The first entry becomes Desktop's default; gateway discovery alone does not choose a Claude-only default. For models whose Copilot `capabilities.limits.max_context_window_tokens` is at least 1,000,000, use an object such as `{ "name": "claude-opus-5-5", "supports1m": true }` to offer Desktop's 1M-context option; plain string entries show only the standard 200K option. Add `"prefer1m": true` to the first entry if its 1M option should be the default. Do not mark 200K models (for example Haiku 4.5) as 1M. Desktop reads local profile changes at launch, so finish active sessions before reopening it. For Claude Code, `CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1` adds available models to its picker but does not change its built-in tier aliases. Use the Opus/Sonnet/Haiku tier variables only when you want to pin or override their model IDs.
...
The checked-in [Claude Code settings example](examples/claude-code/settings.json) starts new sessions with Claude Code's built-in `opus[1m]` alias. Gateway discovery adds the available Claude models to the picker; Opus and Sonnet are not separately pinned, while the Haiku tier explicitly uses GPT-6 Luna. The 1M alias uses Claude Code's extended-context selection for the available Opus model; the gateway resolves the `[1m]` suffix to the upstream model ID. The example keeps `xhigh` effort (including the Opus 5.5 per-model preference) and fullscreen TUI, and disables usage telemetry, error reporting, feedback commands, and session surveys without disabling automatic updates. Copy it to your user-level Claude Code settings if you want those preferences across projects. It intentionally contains **no gateway key**: supply `ANTHROPIC_AUTH_TOKEN` separately in your local environment or another untracked user-level setting, and never commit it.

The example enables the official TypeScript and Pyright LSP plugins. Install `typescript-lsp@claude-plugins-official` and `pyright-lsp@claude-plugins-official` at user scope and ensure the separate `typescript-language-server` and `pyright-langserver` executables are on your PATH; enabling a plugin alone does not install its language server. See [Claude Code's code intelligence plugins](https://code.claude.com/docs/en/discover-plugins#code-intelligence).
...
- Claude Code disables its native MCP tool search by default when `ANTHROPIC_BASE_URL` points to a custom gateway. Claude models on this gateway can handle native deferred-tool search, but if the same Claude Code installation also routes the Haiku tier to GPT-6 Luna, leave the process-wide `ENABLE_TOOL_SEARCH` unset: it can prevent the gateway from receiving the deferred tool definitions Luna needs. For GPT Responses tool search, use the separate [MCP bridge](#gpt-tool-search). See [Claude Code's tool search settings](https://code.claude.com/docs/en/mcp#configure-tool-search).
...
- If `ANTHROPIC_DEFAULT_HAIKU_MODEL` points to a non-Claude ID such as `gpt-6-luna` and you want Haiku's 200K context behavior, set `CLAUDE_CODE_MAX_CONTEXT_TOKENS` to `"200000"` in Claude Code's user-level `settings.json`. Claude Code uses this limit for unrecognized model IDs with proactive compaction, but not for recognized Claude IDs unless compaction is disabled. It also applies to any other unrecognized IDs used in that Claude Code installation; this is a client-side limit, not a gateway-side cap, and does not change Codex's model limits. Do not use a global `CLAUDE_CODE_AUTO_COMPACT_WINDOW` for this purpose; it also changes compaction for the main Claude models. See [Claude Code's gateway model window guidance](https://code.claude.com/docs/en/model-config#correct-the-window-for-a-gateway-or-custom-model-id).
